...
app.initializers.add('imeepo-checkin', () => {
  extend(IndexPage.prototype, 'sidebarItems', function(items) {
    if(app.session.user!==null && app.forum.attribute('allowCheckIn')===true){
      ...
      if(canCheckin===true){
        checkinButtonText = app.translator.trans('imeepo-checkin.forum.check-in');
              ...
              if(forumCheckinSuccessPromptText!=="" || forumCheckinSuccessPromptRewardText!==""){
                if(forumCheckinSuccessPromptType===1){
                  if(canCheckinRecheck===false){
                    ...
                    const failedTextAlertKey = app.alerts.show(Alert, { type: 'error' }, app.translator.trans('imeepo-checkin.forum.check-in-failed')+" "+app.translator.trans('imeepo-checkin.forum.try-again-later'));
                  }
        ...
        checkinButtonText = totalContinuousCheckIn<=1?app.translator.trans('imeepo-checkin.forum.checked-in-day', {count: totalContinuousCheckIn}):app.translator.trans('imeepo-checkin.forum.checked-in-days', {count: totalContinuousCheckIn});
        items.add(itemName, Button.component({
