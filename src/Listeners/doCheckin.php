...
namespace imeepo\checkin\Listeners;

use Flarum\Settings\SettingsRepositoryInterface;
use Illuminate\Contracts\Events\Dispatcher;
use Flarum\User\User;
use Flarum\User\Event\Saving;
use imeepo\checkin\Event\checkinUpdated;
    ...
    public function checkinSaved(Saving $event){
        ...
        if($allowCheckIn){
            ...
            if (array_key_exists('canCheckin', $attributes)) {
                $canCheckin = true;
                $timezone = intval($this->settings->get('imeepo-forum-checkin.checkinTimeZone', 0));
